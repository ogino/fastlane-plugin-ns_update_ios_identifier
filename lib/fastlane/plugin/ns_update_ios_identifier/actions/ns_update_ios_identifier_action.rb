...
require_relative "../helper/ns_update_ios_identifier_helper"

module Fastlane
  module Actions
    class NsUpdateIosIdentifierAction < Action
      def self.run(params)
        require "plist"
        require "xcodeproj"
        ...
        if app_id_equals_bundle_id
          ...
          configs = configs.select { |obj| resolve_path(obj.build_settings[info_plist_key], params[:xcodeproj]).include? params[:plist_path] }
